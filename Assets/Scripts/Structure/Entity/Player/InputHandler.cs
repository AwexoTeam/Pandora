...
public class InputHandler : MonoBehaviour {
    ...
    private void Update()
    {
        ...
        {
            if (uIHandler.currState == "inv") { uIHandler.currState = ""; }
            else { uIHandler.currState = "inv"; }
        }

        if (Input.mouseScrollDelta.y > 0)
        {
            entityController.inventory.ChangeSelected(SelectState.UP);
        }
        else if(Input.mouseScrollDelta.y < 0)
        {
            entityController.inventory.ChangeSelected(SelectState.DOWN);
        }

        if (Input.GetMouseButtonUp(0))
        {
            if(entityController.inventory.currItem != null && !entityController.isInWindow)
            {
                Item i = entityController.inventory.currItem.item;
                foreach (IItemFunction func in i.functions)
                {
                    func.OnSelectedUse(entityController);
                }
            }
        }
    }
