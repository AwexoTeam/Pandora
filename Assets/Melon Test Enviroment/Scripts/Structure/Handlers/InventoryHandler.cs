...
public class InventoryHandler : MonoBehaviour {
    ...
    public GameObject currHoldingItem;
    ...
    {
        foreach (Item item in gameDatabase.Item_Database)
        {
    ...
    private void UpdateCurrItem()
    {
        ...
        {
            currItem = new ItemEntity();
        }

        if (currItem != null)
        {
            if (currItem.item.modelInfo != null)
            {
                Debug.Log("#2");
                if (currHoldingItem != null) { Destroy(currHoldingItem); }
                Debug.Log(currItem.item.modelInfo.model.name);
                currHoldingItem = Instantiate(currItem.item.modelInfo.model);
                currHoldingItem.transform.parent = Hand.transform.parent;

                ModelInfo info = currItem.item.modelInfo;
                currHoldingItem.transform.localPosition = info.position_Offset;
                currHoldingItem.transform.localScale = info.scale_Offset;
                currHoldingItem.transform.localRotation = info.rotation_Offset;
            }
            else
            {
                Debug.Log("#4");
